...
\"(\\.|[^\\\"\n])*\"        { yylval.node = createString(yylineno, yytext); return STRING; }
\'(\\.|[^\\\'\n])*\'        { yylval.node = createString(yylineno, yytext); return STRING; }
[0-9]+                      { yylval.node = createNumber(yylineno, yytext); return NUMBER; }
[0-9]+\.[0-9]               { yylval.node = createNumber(yylineno, yytext); return NUMBER; }
[A-Za-z_][A-Za-z0-9_]*      { yylval.node = createIdentifier(yylineno, yytext); return IDENTIFIER; }
